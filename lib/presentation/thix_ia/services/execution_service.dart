...
class ExecutionService {
  ...
  Future<ExecutionProject> refreshHealth(String projectCode) async {
    ...
    final double doneRate = tasks.isEmpty
        ? 0.0
        : (tasks.where((t) => t.status == 'done').length / tasks.length * 100)
            .toDouble();
    final double goalsRate = goals.isEmpty
        ? 0.0
        : (goals.map((g) => g.progress).fold(0.0, (a, b) => a + b) /
                goals.length)
            .toDouble();
    final double complianceRate = compliance.isEmpty
        ? 100.0
        : (compliance.where((c) => c.status == 'valid').length /
                compliance.length *
                100)
            .toDouble();
    final double riskPenalty =
        (risks.where((r) => r.level >= 12).length * 5).toDouble();

    final Map<String, double> dimensions = {
      'market': goalsRate,
      'product': doneRate,
      'finance':
          (complianceRate * 0.5 + (100 - riskPenalty).clamp(0, 100) * 0.5)
              .toDouble(),
    ...
    // ★★★ CORRECTION : calcul réel depuis thix_execution_finances ★★★
    final finance = ExecutionUtils.computeFinanceFromTransactions(transactions);
    final double treasury = finance['treasury'] ?? 0.0;
    final double burn = finance['burnRate'] ?? 0.0;
    final double runway = finance['runwayMonths'] ?? 0.0;
    final double mrr = finance['mrr'] ?? 0.0;

    final lateTasks = tasks.where((t) => t.isLate).toList();
    final atRiskGoals =
        goals.where((g) => g.isAtRisk).map((g) => g.title).toList();
  ...
  /// Appelé après chaque ajout de transaction pour rafraîchir le dashboard
  Future<ExecutionProject> recalculateFinance(String projectCode) async {
    return refreshHealth(projectCode);
  }
