...
class _FinancePageState extends ConsumerState<FinancePage> {
  ...
  Future<void> _startFinance() async {
    final inputs = {
      ...
      'deterministic': true, // §12 cahier : calculs déterministes backend Python, jamais LLM seul
    };
    await ref.read(analysesProvider.notifier).startFinanceAnalysis(inputs);
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Modèle financier lancé - calculs déterministes')));
  }
        ...
        children: [
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              SizedBox(height: 12),
              Row(children: [Expanded(child: _Field(controller: _revenueController, label: 'Revenu mensuel')), SizedBox(width: 12), Expanded(child: _Field(controller: _costController, label: 'Coût mensuel'))]),
              SizedBox(height: 12),
              _Field(controller: _growthController, label: 'Croissance mensuelle %'),
              SizedBox(height: 16),
              Container(padding: EdgeInsets.all(10), decoration: BoxDecoration(color: ThixPolicy.info.withOpacity(0.08), borderRadius: BorderRadius.circular(8)), child: Row(children: [Icon(Icons.info_outline_rounded, size: 16, color: ThixPolicy.info), SizedBox(width: 8), Expanded(child: Text('Calculs déterministes côté backend Python (NPV, IRR, Break-even). Scénarios base/optimiste/pessimiste.', style: ThixPolicy.captionStyle.copyWith(color: ThixPolicy.info)))])),
              SizedBox(height: 16),
              SizedBox(width: double.infinity, child: ElevatedButton.icon(onPressed: _startFinance, icon: Icon(Icons.calculate_rounded, color: Colors.white), label: Text('Calculer modèle financier'), style: ElevatedButton.styleFrom(backgroundColor: ThixPolicy.success))),
            ]),
          ),
          if (financeAnalyses.isEmpty)
            Padding(padding: EdgeInsets.all(24), child: Text('Aucun modèle financier. Entrez vos paramètres pour générer prévisionnel 3 ans.', style: ThixPolicy.bodySmallStyle))
          else
