// lib/presentation/network/following_list_page.dart
import 'dart:async';
...
class _FollowingValidators {
  _FollowingValidators._();
...
class FollowingProfile {
  ...
  factory FollowingProfile.fromMap(Map<String, dynamic> map) {
...
class FollowingState {
  final List<FollowingProfile> profiles;
  final bool hasMore;
  ...
  FollowingState copyWith({
    List<FollowingProfile>? profiles,
    bool? hasMore,
    bool? isLoading,
  }) {
...
class FollowingNotifier extends AutoDisposeFamilyAsyncNotifier<FollowingState, String> {
  int _offset = 0;

  @override
  Future<FollowingState> build(String arg) async {
    _offset = 0;
    return _loadInitial();
  }

  Future<FollowingState> _loadInitial() async {
    debugPrint('[Following] 📥 Loading initial for ${arg.substring(0, 8)}...');
    try {
      // ÉTAPE 1 : IDs des utilisateurs suivis (follower_id = userId)
      ...
      final followingIds = (followsRes as List)
          .map((e) => e['following_id'].toString())
          .where((id) => id.isNotEmpty)
          .toList();

      if (followingIds.isEmpty) {
        debugPrint('[Following] ✓ No following');
        return const FollowingState(hasMore: false);
      }
      ...
      final profiles = (profilesRes as List)
          .map((p) => FollowingProfile.fromMap(p as Map<String, dynamic>))
          .toList();

      _offset = profiles.length;
      debugPrint('[Following] ✓ Loaded ${profiles.length} profiles');
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      debugPrint('[Following] ✓ Loaded ${unique.length} more (total: ${current.profiles.length + unique.length})');
    } catch (e) {
      debugPrint('[Following] ❌ Load more error: $e');
...
final followingProvider = AsyncNotifierProvider.autoDispose
    .family<FollowingNotifier, FollowingState, String>(FollowingNotifier.new);
...
class _FollowingListPageState extends ConsumerState<FollowingListPage> {
  ...
  void initState() {
    ...
    debugPrint('[Following] 🏷️ Opened for ${widget.userId.substring(0, 8)}...');
  }

  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 400) {
      ref.read(followingProvider(widget.userId).notifier).loadMore();
    }
  }
  ...
  Widget build(BuildContext context) {
    final asyncState = ref.watch(followingProvider(widget.userId));
  ...
  Widget _buildBody(FollowingState state) {
    ...
    return RefreshIndicator(
      ...
      onRefresh: () => ref.read(followingProvider(widget.userId).notifier).refresh(),
      child: ListView.builder(
        ...
        itemBuilder: (context, i) {
          ...
          return _FollowingTile(profile: filtered[i]);
        },
      ),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _FollowingValidators.sanitize(error),
              textAlign: TextAlign.center,
              style: ThixPolicy.bodyStyle.copyWith(color: ThixPolicy.textSecondary),
            ),
            const SizedBox(height: 24),
            ElevatedButton.icon(
              onPressed: () => ref.invalidate(followingProvider(widget.userId)),
                ...
                child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.person_add_alt_1_rounded, size: 64, color: ThixPolicy.primary),
                    ),
                    const SizedBox(height: 24),
                    Text('Aucun abonnement', style: ThixPolicy.h3Style.copyWith(fontWeight: ThixPolicy.bold)),
                    const SizedBox(height: 8),
                    Text(
                    ...
                    ElevatedButton.icon(
                      ...
                      label: const Text('Explorer le réseau'),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: ThixPolicy.primary,
                        foregroundColor: Colors.white,
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(ThixPolicy.rFull)),
...
class _FollowingTile extends ConsumerStatefulWidget {
  final FollowingProfile profile;
...
class _FollowingTileState extends ConsumerState<_FollowingTile> {
  ...
  bool get _isFollowing => _isFollowingOverride ?? true; // Par défaut true (on est dans les abonnements)
  ...
  Future<void> _toggleFollow() async {
    ...
    if (currentUserId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Vous devez être connecté'), backgroundColor: ThixPolicy.danger, behavior: SnackBarBehavior.floating),
      );
      return;
    }
    ...
    try {
      ...
      if (wasFollowing) {
        await supa
            ...
            .timeout(_kRequestTimeout);
        debugPrint('[Following] ✓ Unfollowed ${widget.profile.userId.substring(0, 8)}');
      } else {
        await supa.from('follows').insert({
          'follower_id': currentUserId,
          'following_id': widget.profile.userId,
        }).timeout(_kRequestTimeout);
        debugPrint('[Following] ✓ Re-followed ${widget.profile.userId.substring(0, 8)}');
      }
    } catch (e) {
      debugPrint('[Following] ❌ Toggle error: $e');
      if (mounted) {
        setState(() => _isFollowingOverride = wasFollowing);
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    final name = _FollowingValidators.sanitize(p.displayName, maxLength: 60);
    final profession = _FollowingValidators.sanitize(p.profession, maxLength: 100);
    final photoUrl = _FollowingValidators.sanitizeUrl(p.photoUrl);
