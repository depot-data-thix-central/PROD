...
// ─────────────────────────────────────────────────────────────
// PALETTE DU POST CARD — 100% Blanc Épuré (Inspiré des maquettes)
// ─────────────────────────────────────────────────────────────
class _PostColors {
...
class _PostCardState extends ConsumerState<PostCard> with AutomaticKeepAliveClientMixin {
  ...
  Widget _buildMediaGrid(List<String> urls) {
    ...
    const spacing = 4.0;
    final radius = BorderRadius.circular(16.0); // Angles bien arrondis comme la maquette
    ...
    if (urls.length == 1) {
      return LayoutBuilder(
        builder: (context, c) {
          final w = c.maxWidth;
          final h = (w * 0.75).clamp(220.0, 480.0); // Format panoramique elegant
    ...
    return ProviderScope(
      ...
      child: Consumer(
        builder: (context, ref, _) {
          ...
          final hasMedia = !post.isRepostCard && (post.hasImages || post.hasVideos);
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(color: const Color(0xFF0A1F44).withOpacity(0.04), blurRadius: 24, offset: const Offset(0, 8))
              ],
            ),
            child: Material(
              color: Colors.transparent,
              child: InkWell(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    // ─── 1. HEADER (Avatar Cercle, Nom, Menu) ───
                    Padding(
                      ...
                      child: Row(
                        children: [
                              ...
                              children: [
                                CircleAvatar(
                                  ...
                                  child: post.authorAvatar == null || post.authorAvatar!.isEmpty
                                      ? const Icon(Icons.person_rounded, size: 20, color: _PostColors.textMuted)
                                      : null,
                                ),
                                  ...
                                  Row(
                                    children: [
                                      Flexible(
                                        child: Text(
                                          post.authorName,
                                          style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 15, color: _PostColors.navyText, letterSpacing: -0.2),
                                          maxLines: 1, overflow: TextOverflow.ellipsis
                                        ),
                                      ),
                                      if (isCertified)
                                        CertificationNameBadge(tier: tier, status: status, showLabel: false, iconSize: 16, padding: const EdgeInsets.only(left: 6))
                                      else if (isLegacyVerified)
                                        const Padding(padding: EdgeInsets.only(left: 6), child: Icon(Icons.verified_rounded, color: Color(0xFFE3B23C), size: 16)),
                                    ],
                                  ),
                          ...
                          _buildContextMenu(post, isOwner, isCurrentUserFree),
                        ],
                      ),
                    ),
                    ...
                    if (post.isRepostCard)
                      const Padding(
                        ...
                        child: Row(children: [Icon(Icons.repeat_rounded, size: 14, color: _PostColors.textMuted), SizedBox(width: 6), Text('a reposté', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _PostColors.textMuted))]),
                      ),
                    ...
                    if (post.isRepostCard && post.repostOfId != null && post.repostOfId!.isNotEmpty) ...[
                      ...
                      Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 20),
                        child: _OriginalPostEmbed(postId: post.repostOfId!),
                      ),
                    ],
                    ...
                    if (post.hasAudio && post.audioUrls.isNotEmpty) ...[
                      const SizedBox(height: 12),
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: _ThixWaveformAudioPlayer(audioUrl: post.audioUrls.first)),
                    ],
                    if (post.postType == 'poll') ...[
                      const SizedBox(height: 12),
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: _buildPollWidget(post)),
                    ] else if (post.postType == 'challenge') ...[
                      const SizedBox(height: 12),
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: _buildChallengeWidget(post)),
                    ],

                    _buildFactCheckBanner(post.isMisinformation, post.factCheckMessage),

                    // ─── 4. MÉDIAS ET ACTION ROW OVERLAP (Design exact des captures) ───
                    if (hasMedia)
                      Stack(
                        clipBehavior: Clip.none,
                        children: [
                          Padding(
                            padding: const EdgeInsets.only(top: 16, left: 16, right: 16, bottom: 28), // Espace en bas pour l'overlap
                            child: _buildMediaGrid([...post.imageUrls, ...post.videoUrls]),
                          ),
                          Positioned(
                            bottom: 6, // Le centre de l'action row chevauche le bas de l'image
                            left: 0, right: 0,
                            child: _buildActionRow(post, isLiked, likesCount, ref, true),
                          )
                        ],
                      )
                    else 
                      Padding(
                        ...
                        child: _buildActionRow(post, isLiked, likesCount, ref, false),
                      )
                  ],
                ),
              ),
  ...
  // ── MENU CONTEXTUEL ──
  Widget _buildContextMenu(NetworkPost post, bool isOwner, bool isCurrentUserFree) {
    return PopupMenuButton<String>(
      icon: const Icon(Icons.more_horiz_rounded, size: 22, color: _PostColors.textMuted),
      color: _PostColors.surface,
      ...
      onSelected: (v) async {
        ...
        switch (v) {
          case 'edit':
            // Logique de modification (conservée)
            break;
          case 'delete':
            final ok = await showDialog<bool>(
              ...
              builder: (ctx) => AlertDialog(
                ...
                actions: [
                  ...
                  ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: _PostColors.coral, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))), child: const Text('Supprimer')),
                ],
              ),
            );
      ...
      itemBuilder: (_) => [
        if (isOwner && !isCurrentUserFree) const PopupMenuItem(value: 'edit', child: Row(children: [Icon(Icons.edit_outlined, size: 18, color: _PostColors.navyText), SizedBox(width: 10), Text('Modifier')])),
        if (isOwner) const PopupMenuItem(value: 'delete', child: Row(children: [Icon(Icons.delete_outline_rounded, size: 18, color: _PostColors.coral), SizedBox(width: 10), Text('Supprimer', style: TextStyle(color: _PostColors.coral))])),
        const PopupMenuItem(value: 'save', child: Row(children: [Icon(Icons.bookmark_border_rounded, size: 18, color: _PostColors.navyText), SizedBox(width: 10), Text('Sauvegarder')])),
        const PopupMenuItem(value: 'hide', child: Row(children: [Icon(Icons.visibility_off_outlined, size: 18, color: _PostColors.navyText), SizedBox(width: 10), Text('Masquer')])),
      ],
    );
  }

  // ── LIGNE D'ACTIONS (J'aime flottant, Avatars + Compteurs) ──
  Widget _buildActionRow(NetworkPost post, bool isLiked, int likesCount, WidgetRef ref, bool isOverlap) {
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: isOverlap ? 28 : 20),
      child: Row(
        children: [
          // 1. Bouton J'aime Cercle Blanc Flottant
          GestureDetector(
            onTap: () async {
              HapticFeedback.selectionClick();
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(color: const Color(0xFF0A1F44).withOpacity(0.08), blurRadius: 15, offset: const Offset(0, 4))
                ]
              ),
              child: Icon(
                isLiked ? Icons.favorite_rounded : Icons.favorite_border_rounded,
                color: isLiked ? _PostColors.coral : _PostColors.textMuted,
                size: 24,
              ),
            ),
          ),
          ...
          // 2. Pile d'Avatars des Likers
          if (likesCount > 0)
            _LikersStack(count: likesCount, postId: post.id),
            
          const Spacer(),
          
          // 3. Bouton Commentaires
          _buildIconBtn(Icons.chat_bubble_outline_rounded, widget.onComment ?? () => _openPostDetails(post.id)),
          const SizedBox(width: 12),
          
          // 4. Bouton Partager
          _buildIconBtn(Icons.share_outlined, widget.onShare),
        ],
      ),
    );
  }
  ...
  Widget _buildIconBtn(IconData icon, VoidCallback? onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon, size: 22, color: _PostColors.textMuted),
      ),
    );
  }
...
// ─────────────────────────────────────────────────────────────
// COMPOSANT : PILE DES AVATARS DES UTILISATEURS (Likers)
// ─────────────────────────────────────────────────────────────
class _LikersStack extends StatelessWidget {
  final int count;
  final String postId;
  
  const _LikersStack({required this.count, required this.postId});
  
  @override
  Widget build(BuildContext context) {
    final displayCount = min(5, count);
    final extra = count - displayCount;
    
    // Génération de couleurs pseudo-aléatoires stylisées pour l'esthétique
    final colors = [
      ...
      children: [
        Row(
          children: List.generate(displayCount, (i) {
            return Align(
              widthFactor: 0.65, // Effet de chevauchement
              child: Container(
                width: 28, height: 28,
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
        ...
        if (extra > 0)
          Padding(
            ...
            child: Text(
              '+$extra', 
              style: const TextStyle(color: _PostColors.textMuted, fontSize: 13, fontWeight: FontWeight.bold)
            ),
          )
      ],
    );
...
// ─────────────────────────────────────────────────────────────
// GALERIE & VIDEO & AUDIO PLAYERS (Restylés mais fonctionnels)
// ─────────────────────────────────────────────────────────────
