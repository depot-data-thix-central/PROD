...
// ─────────────────────────────────────────────────────────────
// COMPOSANT PRINCIPAL — POST CARD (Épuré & Transparent)
// ─────────────────────────────────────────────────────────────
...
class _PostCardState extends ConsumerState<PostCard> with AutomaticKeepAliveClientMixin {
  ...
  List<InlineSpan> _parseContent(String content, TextStyle baseStyle, int depth) {
    ...
    for (final match in _richContentRegex.allMatches(content)) {
      ...
      if (match.group(1) != null) {
        ...
        spans.add(TextSpan(
          text: '@$value',
          style: baseStyle.copyWith(color: _PulseColors.navy, fontWeight: ThixPolicy.semiBold),
          recognizer: r,
        ));
          ...
          GestureDetector(
            ...
            child: Padding(
              ...
              child: Text(
                ...
                style: const TextStyle(color: _PulseColors.navy, fontSize: 12.5, fontWeight: FontWeight.w700),
              ),
            ),
          ),
      ],
  ...
  Widget _buildMediaGrid(List<String> urls) {
    ...
    const spacing = 2.0;
    final radius = BorderRadius.circular(ThixPolicy.rLg); 
    ...
    Widget mediaTile(String url, {double? width, double? height}) {
      ...
      return GestureDetector(
        ...
        child: CachedNetworkImage(
          ...
          placeholder: (context, url) => Container(color: ThixPolicy.surfaceStrong, child: const Center(child: SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2, color: ThixPolicy.primary)))),
          errorWidget: (context, url, error) => Container(color: ThixPolicy.surfaceStrong, child: const Icon(Icons.broken_image_outlined, color: ThixPolicy.textMuted)),
        ),
      );
    }
    ...
    if (urls.length == 2) {
      return SizedBox(height: 200, child: Row(children: [cell(0, 200), const SizedBox(width: spacing), cell(1, 200)]));
    }
    ...
    return SizedBox(
      ...
      child: Row(
        children: [
          Expanded(flex: 3, child: ClipRRect(borderRadius: radius, child: mediaTile(urls[0], height: 240, width: double.infinity))),
  ...
  Widget _buildPollWidget(NetworkPost post) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: ThixPolicy.border, width: 0.5),
      ),
      child: Column(
        ...
        children: [
          const Row(
            children: [
              Icon(Icons.poll_rounded, size: 18, color: _PulseColors.navy),
              ...
              child: Material(
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: ThixPolicy.surfaceStrong.withValues(alpha: 0.3),
                      borderRadius: BorderRadius.circular(ThixPolicy.rMd),
                    ),
                    child: Stack(
                      children: [
                        Positioned.fill(
                          child: FractionallySizedBox(
                            ...
                            child: Container(
                              decoration: BoxDecoration(
                                color: _PulseColors.navy.withValues(alpha: 0.15),
                        ...
                        Row(
                          children: [
                            ...
                            Text('${(pct * 100).toStringAsFixed(0)}%', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: _PulseColors.navy)),
                          ],
                        ),
                      ],
                    ),
          ...
          Row(
            children: [
              Container(
                ...
                decoration: const BoxDecoration(color: _PulseColors.gold, shape: BoxShape.circle),
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              style: OutlinedButton.styleFrom(
                foregroundColor: _PulseColors.navy,
                side: const BorderSide(color: _PulseColors.navy),
  ...
  Widget _buildFactCheckBanner(bool isMisinformation, String? message) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: ThixPolicy.danger.withValues(alpha: 0.045), borderRadius: BorderRadius.circular(ThixPolicy.rMd), border: const Border(left: BorderSide(color: ThixPolicy.danger, width: 3))),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Padding(padding: EdgeInsets.only(top: 1), child: Icon(Icons.shield_outlined, color: ThixPolicy.danger, size: 17)),
          const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Fact-Check THIX IA', style: TextStyle(color: ThixPolicy.danger, fontWeight: FontWeight.w800, fontSize: 11.5, letterSpacing: 0.2)), const SizedBox(height: 4), Text(message, style: const TextStyle(color: ThixPolicy.textMain, fontSize: 12, height: 1.4))])),
        ],
      ),
    );
  }

  // ── AVATARS "PULSE" (Ceux qui ont liké) ──
  Widget _buildLikedByAvatars(bool isLiked, int likesCount, String? currentUserAvatar) {
    if (likesCount == 0 && !isLiked) return const SizedBox.shrink();

    List<String> displayAvatars = [];
    if (isLiked && currentUserAvatar != null) {
      displayAvatars.add(currentUserAvatar);
    }
    // (Dans un contexte réel, vous ajouteriez ici les URLs des autres profils)

    int remaining = isLiked ? (likesCount - 1) : likesCount;

    return Padding(
      padding: const EdgeInsets.only(top: 12.0, bottom: 4.0),
      child: Row(
        children: [
          if (displayAvatars.isNotEmpty || remaining > 0)
            SizedBox(
              width: (16.0 * displayAvatars.length) + (remaining > 0 ? 28.0 : 0),
              height: 24,
              child: Stack(
                children: [
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: CircleAvatar(
                          ...
                          backgroundImage: CachedNetworkImageProvider(displayAvatars[i]),
                        ),
                      ),
                    ),
                  if (remaining > 0)
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: CircleAvatar(
                          radius: 10,
                          backgroundColor: ThixPolicy.surfaceStrong,
                          child: Text('+$remaining', style: const TextStyle(fontSize: 8, fontWeight: FontWeight.w800, color: ThixPolicy.textSecondary)),
                        ),
                      ),
                    ),
                ],
          ...
          Expanded(
            child: Text(
              isLiked 
                ? (remaining > 0 ? 'Vous et $remaining autres' : 'Vous avez envoyé une impulsion')
                : '$likesCount impulsions',
  ...
  Widget _buildVerticalAction({required IconData icon, required VoidCallback onTap, Color? color}) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0),
      child: InkWell(
        onTap: onTap,
        borderRadius: BorderRadius.circular(20),
        child: Icon(icon, size: 24, color: color ?? ThixPolicy.textSecondary),
      ),
    );
  }
    ...
    return ProviderScope(
      ...
      child: Consumer(
        builder: (context, ref, _) {
          ...
          final currentUserAvatar = currentUserProfile?['avatar_url']?.toString();
          ...
          return Container(
            // Épuré : sans bordures excessives, juste un séparateur en bas
            decoration: const BoxDecoration(
              color: Colors.transparent,
              border: Border(bottom: BorderSide(color: ThixPolicy.surfaceStrong, width: 1.0)),
            ),
            child: Material(
              color: Colors.white.withValues(alpha: 0.98), // Glassmorphism très léger
              child: InkWell(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                              ...
                              children: [
                                Container(
                                  ...
                                  child: CircleAvatar(
                                    ...
                                    child: post.authorAvatar == null || post.authorAvatar!.isEmpty ? const Icon(Icons.person_rounded, size: 19, color: _PulseColors.navy) : null,
                                  ),
                                ),
                                  ...
                                  Positioned(
                                    ...
                                    child: GestureDetector(
                                      onTap: () async {
                                        ...
                                        } catch (_) { if (mounted) setState(() => _isFollowingLocal = false); } finally { if (mounted) setState(() => _followBusy = false); }
                                      },
                                      child: Container(
                                        width: 20, height: 20,
                                        decoration: BoxDecoration(shape: BoxShape.circle, color: _PulseColors.navy, border: Border.all(color: Colors.white, width: 2.5)),
                            ...
                            child: GestureDetector(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      Flexible(child: Text(post.authorName, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: ThixPolicy.textMain), maxLines: 1, overflow: TextOverflow.ellipsis)),
                                      if (isCertified) CertificationNameBadge(tier: tier, status: status, showLabel: false, iconSize: 16, padding: const EdgeInsets.only(left: 5))
                                      else if (isLegacyVerified) const Padding(padding: EdgeInsets.only(left: 4), child: Icon(Icons.verified_rounded, color: _PulseColors.gold, size: 15)),
                                    ],
                                  ),
                                  if (post.authorTitle != null && post.authorTitle!.isNotEmpty) Text(post.authorTitle!, style: const TextStyle(fontSize: 10.5, color: ThixPolicy.textSecondary), maxLines: 1, overflow: TextOverflow.ellipsis),
                          ...
                          PopupMenuButton<String>(
                            icon: const Icon(Icons.more_horiz_rounded, size: 20, color: ThixPolicy.textSecondary),
                                ...
                                case 'edit':
                                  ...
                                  final newContent = await showDialog<String>(
                                    ...
                                    builder: (dialogContext) => AlertDialog(
                                      ...
                                      title: const Row(children: [Icon(Icons.edit_rounded, color: _PulseColors.navy), SizedBox(width: 8), Text('Modifier la publication', style: TextStyle(fontSize: 16))]),
                                      content: TextField(
                                        controller: controller, maxLines: 6, autofocus: true,
                                        decoration: InputDecoration(hintText: 'Modifier votre texte...', border: OutlineInputBorder(borderRadius: BorderRadius.circular(ThixPolicy.inputRadius))),
                                      ),
                                      actions: [
                                        ...
                                        ElevatedButton(
                                          ...
                                          style: ElevatedButton.styleFrom(backgroundColor: _PulseColors.navy, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(ThixPolicy.rSm))),
                            ...
                            itemBuilder: (_) => [
                              if (isOwner && !isCurrentUserFree) const PopupMenuItem(value: 'edit', child: Row(children: [Icon(Icons.edit_outlined, size: 18, color: ThixPolicy.textMain), SizedBox(width: 10), Text('Modifier')])),
                      ...
                      if (post.isRepostCard)
                        const Padding(
                          ...
                          child: Row(children: [Icon(Icons.repeat_rounded, size: 14, color: ThixPolicy.textSecondary), SizedBox(width: 6), Text('a reposté', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: ThixPolicy.textSecondary))]),
                        ),

                      // ─── DISPOSITION CORPS & ACTIONS VERTICALES LATÉRALES ───
                      Row(
                        ...
                        children: [
                          ...
                          // ── ACTIONS VERTICALES (À droite du contenu) ──
                          Column(
                            ...
                            children: [
                              ...
                              _buildVerticalAction(
                                ...
                                color: post.isSaved ? _PulseColors.navy : null,
                                onTap: widget.onSave ?? () async => await ref.read(postItemProvider.notifier).toggleSave(),
                              ),
                            ],
                          ),
                        ],
                      ...
                      // ── SECTION AVATARS "LIKED BY" (En dessous du bloc) ──
                      _buildLikedByAvatars(isLiked, likesCount, currentUserAvatar),
                    ],
                  ),
                ),
              ),
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: _PulseColors.navy, foregroundColor: Colors.white), child: const Text('Reposter')),
        ],
      ),
    );
...
// ─────────────────────────────────────────────────────────────
// ICÔNE PULSE VERTICALE
// ─────────────────────────────────────────────────────────────
class _PulseVerticalIcon extends StatefulWidget {
  final bool isActive;
  final VoidCallback onTap;
  const _PulseVerticalIcon({required this.isActive, required this.onTap});

  @override
  State<_PulseVerticalIcon> createState() => _PulseVerticalIconState();
}

class _PulseVerticalIconState extends State<_PulseVerticalIcon> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(covariant _PulseVerticalIcon oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.isActive != oldWidget.isActive) {
      if (widget.isActive) _controller.forward(from: 0); else _controller.reverse();
    }
  }
            ...
            return SizedBox(
              ...
              child: Stack(
                ...
                children: [
                  if (_burst.value > 0)
                    Transform.scale(
                      scale: 1.0 + (_burst.value * 0.4),
                      child: Container(
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                          gradient: RadialGradient(colors: [_PulseColors.gold.withValues(alpha: 0.3 * (1 - _burst.value)), Colors.transparent]),
                        ),
                      ),
                    ),
                  Icon(
                    widget.isActive ? Icons.bolt_rounded : Icons.bolt_outlined,
                    size: 26,
                    color: widget.isActive ? _PulseColors.gold : ThixPolicy.textSecondary,
                  ),
                ],
              ),
            );
...
// ─────────────────────────────────────────────────────────────
// LECTEUR AUDIO ÉPURÉ (Design Blanc / Harmonite)
// ─────────────────────────────────────────────────────────────
...
class _ThixWaveformAudioPlayerState extends State<_ThixWaveformAudioPlayer> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: ThixPolicy.s12, vertical: ThixPolicy.s12),
      decoration: BoxDecoration(
        color: Colors.white.withValues(alpha: 0.6), // Glassmorphism subtil
        borderRadius: BorderRadius.circular(ThixPolicy.rLg),
        border: Border.all(color: ThixPolicy.border, width: 0.5),
      ),
      child: Row(
        children: [
          GestureDetector(
            ...
            child: Container(
              width: 44, height: 44,
              decoration: const BoxDecoration(color: _PulseColors.navy, shape: BoxShape.circle),
                ...
                return SizedBox(
                  ...
                  child: Row(
                    ...
                    children: List.generate(barCount, (index) {
                      ...
                      return Container(
                        width: barWidth,
                        height: 36 * baseHeight,
                        margin: const EdgeInsets.only(right: spacing),
                        decoration: BoxDecoration(
                          color: isPlayed ? _PulseColors.gold : ThixPolicy.surfaceStrong,
      ...
      builder: (context, snap) {
        if (snap.connectionState == ConnectionState.waiting) {
          return Container(
            ...
            decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.6), borderRadius: BorderRadius.circular(ThixPolicy.rLg), border: Border.all(color: ThixPolicy.border, width: 0.5)),
            child: const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2.5, color: _PulseColors.navy)),
          );
        }
        ...
        return Container(
          decoration: BoxDecoration(color: ThixPolicy.surfaceStrong.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(ThixPolicy.rLg), border: Border.all(color: ThixPolicy.border, width: 0.5)),
                    ...
                    Row(
                      children: [
                        CircleAvatar(
                          ...
                          child: original.authorAvatar == null || original.authorAvatar!.isEmpty ? const Icon(Icons.person, size: 16, color: _PulseColors.navy) : null,
                        ),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Row(
                            children: [
                              ...
                              if (originalIsCertified)
                                ...
                                const Padding(
                                  ...
                                  child: Icon(Icons.verified_rounded, color: _PulseColors.gold, size: 14)
                                ),
                            ],
                          ),
                        ),
