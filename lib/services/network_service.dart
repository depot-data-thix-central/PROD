...
// LOGGING
// ============================================================================
class _NetworkServiceLogger {
  static void info(String message, [Map<String, dynamic>? data]) {
    if (kDebugMode) debugPrint('[NetworkService][INFO] $message ${data ?? ''}');
  }
  static void warn(String message, [Map<String, dynamic>? data]) {
    debugPrint('[NetworkService][WARN] $message ${data ?? ''}');
  }
  static void error(String message, [Map<String, dynamic>? data]) {
    debugPrint('[NetworkService][ERROR] $message ${data ?? ''}');
  }
}
...
class NetworkService extends ChangeNotifier {
  ...
  Future<List<NetworkPost>> getFeedPosts({
    ...
    try {
      ...
      _NetworkServiceLogger.error('getFeedPosts timeout', {'type': type, 'offset': safeOffset});
      return [];
    } catch (e, stack) {
      _NetworkServiceLogger.error('getFeedPosts failed', {'type': type, 'error': '$e', 'stack': stack.toString()});
      return [];
    }
  }
    ...
    for (int attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        if (attempt > 0) {
          ...
          _NetworkServiceLogger.info('Smart feed retry', {'attempt': attempt, 'delay': '$delay'});
        ...
        if (res == null) throw Exception('RPC returned null');
        if (res is! List) throw Exception('RPC returned invalid type: ${res.runtimeType}');

        final posts = _parsePostsSafely(res);
        _NetworkServiceLogger.info('Smart feed loaded', {'count': posts.length, 'attempt': attempt, 'seed': seed});

        return posts;
      } catch (e) {
        lastError = e;
        _NetworkServiceLogger.warn('Smart feed attempt failed', {'attempt': attempt, 'error': '$e'});
        if (e.toString().contains('400') || e.toString().contains('401') || e.toString().contains('403')) break;
      }
    }

    _NetworkServiceLogger.warn('Smart feed failed, falling back to recent', {'error': '$lastError'});
  ...
  List<NetworkPost> _parsePostsSafely(List<dynamic> data) {
    ...
    if (invalidCount > 0) {
      _NetworkServiceLogger.warn('Invalid posts filtered', {'invalid': invalidCount, 'valid': posts.length});
    }

    return posts;
  }

  Future<List<NetworkPost>> _fetchRecentFeed({required int limit, required int offset}) async {
    try {
      final res = await _supabase
          .from('posts_view')
          .select()
  ...
  Future<List<NetworkPost>> _fetchNetworkFeed({required String uid, required int limit, required int offset}) async {
  ...
  Future<NetworkCommunity> createCommunity({
    ...
    try {
      ...
      if (existing != null) throw Exception('Une communauté avec ce nom existe déjà.');

      final sanitizedDesc = description != null ? _NetworkValidators.sanitizeText(description, maxLength: 2000) : null;
  ...
  Future<void> _verifyPostOwnership(String postId) async {
    try {
      ...
      if (ownerId != currentUserId) throw Exception('Permission refusée : vous n\'êtes pas le propriétaire');
    } on TimeoutException {
  ...
  Future<void> pinPost(String postId) async {
    ...
    await _supabase.from('posts').update({'is_pinned': false}).eq('user_id', currentUserId).eq('is_pinned', true).timeout(_requestTimeout);
  ...
  Future<NetworkPost?> getPinnedPost(String userId) async {
    try {
      final res = await _supabase.from('posts_view').select().eq('user_id', userId).eq('is_pinned', true).maybeSingle().timeout(_requestTimeout);
  ...
  Future<List<NetworkPost>> getPinnedPosts(String userId) async {
    try {
      final res = await _supabase.from('posts_view').select().eq('user_id', userId).eq('is_pinned', true).order('created_at', ascending: false).timeout(_requestTimeout);
  ...
  Future<({bool liked, int likesCount})> togglePostLike(String postId) async {
    ...
    try {
      final res = await _supabase.rpc('rpc_toggle_post_like', params: {'p_post_id': postId}).timeout(_requestTimeout);
      ...
      notifyListeners();
      _NetworkServiceLogger.info('Like toggled', {'postId': postId, 'liked': liked, 'count': count});
      ...
      _NetworkServiceLogger.warn('Like RPC failed, using fallback', {'postId': postId, 'error': '$e'});
      try {
        final existing = await _supabase.from('post_likes').select('post_id').eq('post_id', postId).eq('user_id', currentUserId).maybeSingle();
        if (existing != null) {
        ...
        _NetworkServiceLogger.error('Like fallback failed', {'postId': postId, 'error': '$fallbackError'});
        rethrow;
      }
    } finally {
  ...
  Future<int> _countLikes(String postId) async {
    try {
      final res = await _supabase.from('post_likes').select('post_id').eq('post_id', postId).timeout(_requestTimeout);
      return (res as List).length;
    } catch (e) {
      _NetworkServiceLogger.error('_countLikes failed', {'postId': postId, 'error': '$e'});
      return 0;
    }
  }
  ...
  Future<void> likePost(String id) async {
    ...
    try {
      await _supabase.from('post_likes').upsert({'post_id': id, 'user_id': currentUserId}, onConflict: 'post_id,user_id', ignoreDuplicates: true).timeout(_requestTimeout);
      final owner = await _getPostOwnerId(id);
      if (owner.isNotEmpty && owner != currentUserId) unawaited(_createNotification(userId: owner, type: 'like', postId: id));
      notifyListeners();
    } catch (e) {
      rethrow;
    }
  ...
  Future<void> unlikePost(String id) async {
    ...
    if (id.trim().isEmpty) return;
    try {
      await _supabase.from('post_likes').delete().eq('post_id', id).eq('user_id', currentUserId).timeout(_requestTimeout);
      notifyListeners();
    } catch (e) {
      rethrow;
    }
  ...
  Future<void> sharePost(String id) async {
    ...
    try {
      ...
      try {
        ...
        _NetworkServiceLogger.error('sharePost failed', {'postId': id, 'error': '$e2'});
      }
    }
  }
  ...
  Future<void> unsavePost(String postId) async {
    if (currentUserId.isEmpty) throw Exception('Non authentifié');
    if (postId.trim().isEmpty) throw Exception('PostId invalide');
    await _supabase.from('saved_posts').delete().eq('post_id', postId).eq('user_id', currentUserId).timeout(_requestTimeout);
    notifyListeners();
  }
  ...
  Future<List<NetworkPost>> getSavedPosts() async {
    if (currentUserId.isEmpty) return [];
    try {
      final res = await _supabase.from('saved_posts').select('post:post_id(*, profiles:user_id(display_name, avatar_url, profession))').eq('user_id', currentUserId).order('saved_at', ascending: false).timeout(_requestTimeout);
      if (res == null || res is! List) return [];
      final posts = <NetworkPost>[];
      for (final item in res) {
        if (item is Map && item['post'] is Map) {
          posts.add(NetworkPost.fromJson(Map<String, dynamic>.from(item['post'] as Map)));
        }
      }
      return posts;
    } catch (e) {
  ...
  Future<NetworkPost?> repostPost(String originalPostId, {String? quote}) async {
    ...
    try {
      final res = await _supabase.rpc('rpc_repost', params: {'p_original_post_id': originalPostId, 'p_quote': sanitizedQuote}).timeout(_requestTimeout);
      Map<String, dynamic> row;
      if (res is List && res.isNotEmpty) row = Map<String, dynamic>.from(res.first as Map);
      else if (res is Map) row = Map<String, dynamic>.from(res);
      else throw Exception('Réponse RPC invalide');
  ...
  Future<List<NetworkPost>> getUserReposts(String userId) async {
    ...
    try {
      final res = await _supabase.from('reposts').select('post:original_post_id(*)').eq('user_id', userId).order('created_at', ascending: false).timeout(_requestTimeout);
      if (res == null || res is! List) return [];
      final posts = <NetworkPost>[];
      for (final item in res) {
        if (item is Map && item['post'] is Map) {
  ...
  Future<List<Comment>> getCommentsWithReplies(String postId) async {
    try {
      final res = await _supabase.from('comments').select('*, profiles!user_id(display_name, avatar_url)').eq('post_id', postId).order('created_at', ascending: true).timeout(_requestTimeout);
      final map = <String, Comment>{ for (final j in res as List) '${j['id']}': Comment.fromJson(j) };
      final roots = <Comment>[];
  ...
  Future<Comment> addComment(
    ...
    }).select('*, profiles!user_id(display_name, avatar_url)').single().timeout(_requestTimeout);
    notifyListeners();
    return Comment.fromJson(res);
  }

  Future<bool> addCommentToPost(String postId, String comment) async {
    try { await addComment(postId, comment); return true; } catch (e) { return false; }
  }
  ...
  Future<List<Map<String, dynamic>>> getComments(String postId) async {
    try {
      ...
      return (res as List).map((e) => {
        'id': e['id'], 'user_id': e['user_id'], 'user_name': e['profiles']?['display_name'], 'user_avatar': e['profiles']?['avatar_url'], 'content': e['content'], 'created_at': e['created_at'],
      }).toList();
    } catch (e) { return []; }
  }
  ...
  Future<bool> updateComment(String commentId, String newContent) async {
    try {
      final comment = await _supabase.from('comments').select('user_id').eq('id', commentId).maybeSingle().timeout(_requestTimeout);
      if (comment == null || comment['user_id'] != currentUserId) throw Exception('Permission refusée');
      final sanitizedContent = _NetworkValidators.sanitizeText(newContent);
      if (sanitizedContent.isEmpty) throw Exception('Contenu invalide');
      await _supabase.from('comments').update({'content': sanitizedContent, 'is_edited': true}).eq('id', commentId).timeout(_requestTimeout);
      notifyListeners();
      return true;
  ...
  Future<void> reportComment(String commentId, String reason, String? details) async {
    ...
    final sanitizedDetails = details != null ? _NetworkValidators.sanitizeText(details, maxLength: 500) : null;
    await _supabase.from('comment_reports').insert({
      'comment_id': commentId, 'reporter_id': uid, 'reason': sanitizedReason, 'details': sanitizedDetails, 'status': 'pending', 'reported_at': DateTime.now().toUtc().toIso8601String(),
    }).timeout(_requestTimeout);
  }
  ...
  Future<bool> likeComment(String commentId) async {
    ...
    try {
      await _supabase.from('comment_likes').upsert({'comment_id': commentId, 'user_id': _uid}, onConflict: 'comment_id,user_id', ignoreDuplicates: true).timeout(_requestTimeout);
      notifyListeners();
      return true;
    } catch (e) { return false; }
  }
  ...
  Future<bool> unlikeComment(String commentId) async {
    if (_uid == null) return false;
    try {
      await _supabase.from('comment_likes').delete().eq('comment_id', commentId).eq('user_id', _uid!).timeout(_requestTimeout);
      notifyListeners();
      return true;
    } catch (e) { return false; }
  }
  ...
  Future<String> createPollPost({required String content, required List<String> options, List<String> images = const []}) async {
    if (currentUserId.isEmpty) throw Exception('Non authentifié');
    final sanitizedContent = _NetworkValidators.sanitizeText(content);
    if (sanitizedContent.isEmpty) throw Exception('Contenu invalide');
    ...
    final res = await _supabase.from('posts').insert({
      'user_id': currentUserId, 'content': sanitizedContent, 'media_urls': sanitizedImages, 'image_urls': sanitizedImages,
      'post_type': 'poll', 'poll_data': {'options': formattedOptions}, 'is_public': true,
    }).select('id').single().timeout(_requestTimeout);
    notifyListeners();
    return res['id'] as String;
  }

  Future<String> createChallengePost({required String title, required String description, required DateTime endDate, List<String> images = const []}) async {
    if (currentUserId.isEmpty) throw Exception('Non authentifié');
    ...
    final res = await _supabase.from('posts').insert({
      'user_id': currentUserId, 'content': sanitizedTitle, 'media_urls': sanitizedImages, 'image_urls': sanitizedImages,
      'post_type': 'challenge', 'challenge_data': {'description': sanitizedDesc, 'end_date': endDate.toUtc().toIso8601String(), 'participants_count': 0}, 'is_public': true,
    }).select('id').single().timeout(_requestTimeout);
    notifyListeners();
    return res['id'] as String;
  }
  ...
  Future<void> votePoll(String postId, int optionIndex) async {
    try {
      ...
      if (optionIndex >= 0 && optionIndex < options.length) {
        ...
        if (!targetVotes.contains(currentUserId)) targetVotes.add(currentUserId);
      ...
      notifyListeners();
    } catch (e) { debugPrint('[PostService] Error votePoll: $e'); }
  }
  ...
  Future<List<NetworkStory>> getActiveStories() async {
    try {
      final res = await _supabase.from('active_stories').select('*').order('created_at', ascending: false).limit(30).timeout(_requestTimeout);
      return (res as List).map((e) => NetworkStory.fromJson(e)).toList();
    } catch (e) { return []; }
  }

  Future<void> createStory(String? mediaUrl, {String? text, String mediaType = 'image', int duration = 24, String? bgColor}) async {
    if (currentUserId.isEmpty) throw Exception('Non authentifié');
    await _supabase.from('stories').insert({
      'user_id': currentUserId, 'media_url': mediaUrl, 'image_url': mediaUrl, 'text_content': text, 'content': text, 'text': text,
      'media_type': mediaType, 'bg_color': bgColor, 'is_active': true, 'expires_at': DateTime.now().toUtc().add(Duration(hours: duration.clamp(6, 48))).toIso8601String(),
    }).timeout(_requestTimeout);
    notifyListeners();
  }
  ...
  Future<void> createHighlight(String name, List<String> storyIds, String? coverImage) async {
    ...
    await _supabase.from('story_highlights').insert({'user_id': currentUserId, 'name': sanitizedName, 'cover_image': _NetworkValidators.sanitizeUrl(coverImage), 'story_ids': storyIds}).timeout(_requestTimeout);
  }
  ...
  Future<List<NetworkConnection>> getMyConnections() async {
    ...
    if (uid.isEmpty) return [];
    try {
      final res = await _supabase.from('follows').select('''created_at, following:profiles!follows_following_id_fkey(id, display_name, avatar_url, profession)''').eq('follower_id', uid).order('created_at', ascending: false).timeout(_requestTimeout);
      return (res as List).map((row) {
        final other = row['following'];
        return NetworkConnection(id: other?['id']?.toString() ?? '', name: other?['display_name']?.toString() ?? 'Utilisateur', avatar: other?['avatar_url']?.toString(), title: other?['profession']?.toString() ?? 'Membre THIX', mutualConnections: 0, status: 'accepted', connectedAt: row['created_at'] != null ? DateTime.tryParse(row['created_at'].toString()) : null);
      }).toList();
    } catch (e) {
      try {
        ...
        return (profiles as List).map((p) => NetworkConnection(id: p['id']?.toString() ?? '', name: p['display_name']?.toString() ?? 'Utilisateur', avatar: p['avatar_url']?.toString(), title: p['profession']?.toString() ?? 'Membre THIX', mutualConnections: 0, status: 'accepted')).toList();
      } catch (e2) { return []; }
    }
  }
  ...
  Future<List<NetworkConnection>> getSuggestedConnections({int limit = 10}) async {
    try {
      final res = await _supabase.rpc('get_suggested_connections', params: {'p_user_id': currentUserId, 'p_limit': limit}).timeout(_requestTimeout);
  ...
  Future<void> followUser(String targetId) async {
    ...
    await _supabase.from('follows').upsert({'follower_id': currentUserId, 'following_id': targetId, 'created_at': DateTime.now().toUtc().toIso8601String()}, onConflict: 'follower_id,following_id').timeout(_requestTimeout);
    try { unawaited(_createNotification(userId: targetId, type: 'follow')); } catch (_) {}
    notifyListeners();
  }
  ...
  Future<List<NetworkCommunity>> getAllCommunities({int limit = 20, int offset = 0}) async {
    final safeLimit = limit.clamp(1, 100);
    final safeOffset = offset < 0 ? 0 : offset;
    try {
      final res = await _supabase.from('communities').select('*').eq('is_active', true).order('members_count', ascending: false).range(safeOffset, safeOffset + safeLimit - 1).timeout(_requestTimeout);
  ...
  Future<void> joinCommunity(String id) async {
    await _supabase.from('community_members').upsert({'community_id': id, 'user_id': currentUserId, 'role': 'member'}, onConflict: 'community_id,user_id', ignoreDuplicates: true).timeout(_requestTimeout);
  }
  ...
  Future<List<Conversation>> getConversations() async {
    try {
      final uid = currentUserId;
      final res = await _supabase.from('messages').select('sender_id, receiver_id, content, created_at, is_read, sender:profiles!messages_sender_id_fkey(display_name, avatar_url), receiver:profiles!messages_receiver_id_fkey(display_name, avatar_url)').or('sender_id.eq.$uid,receiver_id.eq.$uid').order('created_at', ascending: false).limit(100).timeout(_requestTimeout);
      ...
      for (final m in res as List) {
        ...
        if (!map.containsKey(otherId)) {
          ...
          map[otherId] = Conversation(id: otherId, otherUserId: otherId, otherUserName: other?['display_name'] ?? 'Utilisateur', otherUserAvatar: other?['avatar_url'], lastMessage: m['content'], lastMessageAt: DateTime.parse(m['created_at']), lastMessageIsFromMe: m['sender_id'] == uid, unreadCount: (m['is_read'] == false && m['receiver_id'] == uid) ? 1 : 0);
        }
      }
  ...
  Future<List<Map<String, dynamic>>> getMessages(String otherId) async {
    final uid = currentUserId;
    try {
      final res = await _supabase.from('messages').select().or('and(sender_id.eq.$uid,receiver_id.eq.$otherId),and(sender_id.eq.$otherId,receiver_id.eq.$uid)').order('created_at', ascending: true).limit(100).timeout(_requestTimeout);
      return (res as List).map((e) => {'id': e['id'], 'content': e['content'], 'is_sent_by_me': e['sender_id'] == uid, 'created_at': DateTime.parse(e['created_at'])}).toList();
    } catch (e) { return []; }
  }
  ...
  Future<Map<String, dynamic>> sendMessage(String receiverId, String content) async {
    ...
    final res = await _supabase.from('messages').insert({'sender_id': currentUserId, 'receiver_id': receiverId, 'content': sanitizedContent, 'is_read': false}).select().single().timeout(_requestTimeout);
    return {'id': res['id'], 'content': res['content'], 'is_sent_by_me': true, 'created_at': DateTime.parse(res['created_at'])};
  }
  ...
  Future<List<NetworkNotification>> getNotifications() async {
    try {
      final res = await _supabase.from('notifications').select('*, profiles!sender_id(display_name, avatar_url)').eq('user_id', currentUserId).order('created_at', ascending: false).limit(50).timeout(_requestTimeout);
  ...
  Future<void> markNotificationAsRead(String notificationId) async {
    ...
    if (uid.isEmpty) return;
    try {
      await _supabase.from('notifications').update({'is_read': true, 'read_at': DateTime.now().toUtc().toIso8601String()}).eq('id', notificationId).eq('user_id', uid).timeout(_requestTimeout);
    } catch (_) {}
  }
  ...
  Future<Map<String, dynamic>?> getUserProfile(String userId) async {
    ...
    try {
      final res = await _supabase.from('profiles').select('id, display_name, avatar_url, cover_url, profession, bio, certification_tier, certification_status, is_verified').eq('id', userId).maybeSingle().timeout(_requestTimeout);
      ...
      final profile = {...res, 'posts_count': results[0], 'followers_count': results[1], 'following_count': results[2]};
  ...
  Future<List<NetworkPost>> getUserPosts(String userId, {int offset = 0, int limit = 15}) async {
    try {
      final res = await _supabase.from('posts_view').select().eq('user_id', userId).order('created_at', ascending: false).range(offset, offset + limit - 1).timeout(_requestTimeout);
      return (res as List).map((e) => NetworkPost.fromJson(Map<String, dynamic>.from(e as Map))).toList();
    } catch (e) { return []; }
  }

  Future<void> markEventInterest(String id) async {
    await _supabase.from('event_interests').upsert({'event_id': id, 'user_id': currentUserId}, onConflict: 'event_id,user_id', ignoreDuplicates: true).timeout(_requestTimeout);
  }
  ...
  Future<String?> uploadImageBytes(Uint8List bytes, {required String fileExtension, String bucket = 'post_images'}) async {
    if (currentUserId.isEmpty) throw Exception('Non authentifié');
    if (!_NetworkValidators.validateFileSize(bytes.length, maxSizeMB: _maxImageSizeMB)) throw Exception('Image trop volumineuse (max ${_maxImageSizeMB}MB)');
    if (!_NetworkValidators.validateFileExtension(fileExtension, _allowedImageExts)) throw Exception('Format image non supporté : $fileExtension');
    ...
    try {
      ...
      if (!kIsWeb) {
        try { finalBytes = await _compressImageAsync(bytes, fileExtension); } catch (_) {}
      }
      ...
      await _supabase.storage.from(bucket).uploadBinary(path, finalBytes, fileOptions: FileOptions(contentType: mimeType, upsert: true)).timeout(const Duration(seconds: 30));
  ...
  Future<Uint8List> _compressImageAsync(Uint8List bytes, String ext) async {
    ...
    if (!kIsWeb) return await compute(_compressImageInIsolate, {'bytes': bytes, 'format': compressFormat});
    return await FlutterImageCompress.compressWithList(bytes, minWidth: 1080, minHeight: 1080, quality: 80, format: compressFormat);
  }
  ...
  static Future<Uint8List> _compressImageInIsolate(Map<String, dynamic> params) async {
    ...
    return await FlutterImageCompress.compressWithList(bytes, minWidth: 1080, minHeight: 1080, quality: 80, format: format);
  }
  ...
  Future<void> _createNotification({required String userId, required String type, String? postId}) async {
    ...
    try {
      await _supabase.from('notifications').insert({'user_id': userId, 'type': type, 'sender_id': currentUserId, 'post_id': postId, 'is_read': false}).timeout(_requestTimeout);
    } catch (_) {}
  }
}
